...
class myClass:
    ...
    def onPositionEvent(self, position):
        ...
        self._logger.info(f"\033[92m[EVENT] onPositionEvent #{self.position_event_count}: {position.position} of {position.contract.localSymbol}\033[0m")
...
async def main():
    ...
    await asyncio.sleep(5)  # wait a bit for order to be processed

    # fut = ib.reqPositionsAsync()
    # await fut
    logger.info(f"Position after:\n{pprint.pformat(ib.positions())}")
    ...
    try:
        await asyncio.wait_for(aw(), timeout=10)
    except TimeoutError:
