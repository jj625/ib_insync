...
async def fetch_historical_data(ib, contract, endDateTime, barSizeSetting, filename):
    logger.info(f"Requesting {barSizeSetting} historical bars for {contract.symbol}... ending {endDateTime}")
    histbars = await ib.reqHistoricalDataAsync(
        contract,
...
async def main():
    ...
    for sym in syms:
        ...
        tasks.append(fetch_historical_data(ib, contract_, endDateTime, '5 secs', filename_5s))
