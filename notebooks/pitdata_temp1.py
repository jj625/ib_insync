...
class PitLogger():
    ...
    def run(self, startTime: datetime.datetime):
        ...
        if self.keepUpToDate:
            ...
            for bar in self.bars:
                self.logfile.write(f"{bar.date.astimezone(tz=None)},{bar.open},{bar.high},{bar.low},{bar.close}"
                    f",{bar.average},{bar.volume:n},{bar.barCount:n},True,{bar.timestamp.astimezone(tz=None)}\n")
                self.logfile.flush()
...
def main():
    ...
    for sym in syms:
        ...
        pl5s = PitLogger(ib, contract_, '60 D', '15 secs', f'./data/{sym}_pit_15s_{dtnow:%Y%m%d_%H%M}.csv', useRTH=True, keepUpToDate=False)
