...
class BarDataResampler():
    ...
    def onBarUpdate(self, bars: List[BarData], hasNewBar: bool):
        ...
        r, h = self.resampler_5s_1m(bars, self.resampled)
...
bars5s: List[BarData] = []
b5s_resampler = BarDataResampler()
...
async def main(ib: IB, host, port, clientId):
    ...
    for sym in syms:
        ...
        bars5s = await ib.reqHistoricalDataAsync(
                contract_,
        ...
        bars1m = await ib.reqHistoricalDataAsync(
                contract_,
                endDateTime=endDateTime,
                durationStr='1 D',
...
class LiveChart(FigureCanvas):
    def __init__(self, parent=None):
        ...
        self.ani = animation.FuncAnimation(fig, self.update_chart, interval=1000)
...
class MainWindow(QMainWindow):
    def __init__(self, host, port, clientId):
        super().__init__()
        self.setWindowTitle("Live Chart Example")
        self.setGeometry(100, 100, 800, 600)
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        layout = QVBoxLayout(central_widget)
        self.live_chart = LiveChart(self)
    ...
    def onConnectButtonClicked(self, _):
        if self.ib.isConnected():
            ...
            loop = util.getLoop()
            self.maintask = loop.create_task( main(self.ib, *self.connectInfo) )
...
if __name__ == '__main__':
    ...
    main_window = MainWindow('127.0.0.1', 7497, 1001)
    main_window.show()
    logger.info("after main_window.show()")
    IB.run()
    main()
